...
will be of the form tuple(|tweet_data[i:i+M]| + |stock_price_data[i:i+M]|). |next_price_dataY| will be a list of our stock prices, starting
...
for i in range(len(tweet_data) - M):
    pastM_dataX.append(tuple(tweet_data[i:i+M] + stock_price_data[i:i+M]))
    next_price_dataY.append(stock_price_data[i+M][0])


#####################################################################################################################################################################
'''
Here we will declare methods to extract features from each tuple in |pastM_dataX|. Note these features
...
for i in range(len(word_and_similars_list)):
    ...
    def totalValue(x):
        ...
        for word_value in similars:
            for j in range(M):
                for tweet in x[j]:
                    value += tweet[0].count(word_value[0]) * sum(tweet[1:]) * word_value[1] * len(tweet[0])
        return value
...
def percent_correct(predictions, actuals):
    count = 0.0
    previous = stock_price_data[int(len(stock_price_data) * TRAIN_SIZE)][0]
    for i in range(len(predictions)):
        if (actuals[i] >= previous and predictions[i] >= previous) or (actuals[i] <= previous and predictions[i] <= previous):
            count += 1
        previous = actuals[i]
    return count / len(predictions)
...
print('LR train percent correct', percent_correct(predY_train, dataY_train))
print('LR test percent correct', percent_correct(predY_test, dataY_test))
...
print('NN train percent correct', percent_correct(predY_train, dataY_train))
print('NN test percent correct', percent_correct(predY_test, dataY_test))
